...
max1=cnt=0
# lower_color1 = np.array([13,83,0])
# upper_color1 = np.array([23,255,67])
while(True):
    ...
    ret, img_binthresh = cv.threshold(img_hsv, 75, 255, cv.THRESH_BINARY)
    # img_threshold1 = cv.inRange(img_hsv,lower_color1,upper_color1)
    contours1, h = cv.findContours(img_binthresh, cv.RETR_TREE, cv.CHAIN_APPROX_NONE)
    if len(contours1)!=0:
